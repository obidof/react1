      ...
      >
        <Box sx={style}>
          ...
          <form onSubmit={handleRegisterSumbit} className='space-y-3'>
            <h1 className='text-3xl text-center text-blue-400'>Rename Category</h1>
            <input onChange={(e) => setIcontext(e.target.value)} className='p-2 border outline-0 rounded-xl w-full inp ' type="file" placeholder='image' defaultValue={category.name} />
            ...
            <button className={`py-2 px-8 rounded-xl cursor-pointer text-white text-xl ${Loading ? 'bg-blue-300 active:scale-100' : 'bg-blue-400 active:scale-90'} flex items-center gap-3`} disabled={Loading}>{Loading ? <MiniLoader /> : "Saqlash"}</button>
          </form>
        </Box>
      </Modal>
    </div>
